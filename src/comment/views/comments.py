...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if author_email:
            email = EmailMessage(
                "دیدگاه جدید",
                "دیدگاه جدیدی برای پست ({}) ثبت شده است که شما نویسنده آن هستید\n {}{}".format(post, current_site, reverse('blog:detail', kwargs={'slug': post.slug})),
                to=[author_email]
            )
            email.send()
        ...
        if user_email:
            email = EmailMessage(
                "تایید ثبت دیدگاه جدید",
                "دیدگاه شما برای پست ({}) باموفقیت ثبت شد\n {}{}".format(post, current_site, reverse('blog:detail', kwargs={'slug': post.slug})),
                to=[user_email]
            )
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                "دیدگاه شما پاسخ جدیدی دریافت کرده است",
                "پاسخ جدیدی برای دیدگاه شما در پست ({}) ثبت شده است\n {}{}".format(post, current_site, reverse('blog:detail', kwargs={'slug': post.slug})),
                to=[parent_email]
            )
            email.send()
