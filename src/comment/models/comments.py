...
class Comment(models.Model):
    ...
    object_id = models.PositiveIntegerField(verbose_name='آیدی')
    ...
    urlhash = models.CharField(
        ...
        unique=True,
        editable=False,
        verbose_name='لینک دیدگاه'
        )
    posted = models.DateTimeField(default=timezone.now, editable=False, verbose_name='ارسال شده در')
    edited = models.DateTimeField(auto_now=True, verbose_name='ویرایش شده در')
    ...
    def __str__(self):
        ...
        if not self.parent:
            return f'دیدگاه توسط {username}: {_content}'
        else:
            return f'پاسخ داده شده توسط {username}: {_content}'
